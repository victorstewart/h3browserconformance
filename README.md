...
- `native/pico_baton/`: expanded conformance `pico_baton` server sources.

## Requirements

- Node.js 22 or newer.
- `openssl` for local test certificates.
- A built `pico_baton` binary for server-backed smoke/browser runs.
- Browser binaries or drivers for browser-specific lanes.

## Picoquic Integration

Build the external conformance `pico_baton` against picoquic:
...
npm run wpt:server-smoke -- --picoquic-root /path/to/picoquic --baton-bin build/pico_baton/pico_baton
```
